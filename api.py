...
async def process_exam_endpoint(request: OMRRequest):
    try:
        # Base64'ten PDF'i çöz
        ...
        # OMR işlemcisini oluştur ve config'i ayarla
        processor = OMRProcessor(request.config)
        
        # Sınav kağıdını işle
        results = processor.process_exam(temp_pdf_path)
        
        # Debug görüntülerini base64'e çevir
        debug_images = {}
        for file in os.listdir('debug_images'):
            if file.endswith(('.png', '.jpg')):
                with open(os.path.join('debug_images', file), 'rb') as img_file:
                    img_data = img_file.read()
                    debug_images[file] = base64.b64encode(img_data).decode('utf-8')
        
        # Sonuçlara debug görüntülerini ekle
        results['debugImages'] = debug_images
